Shader "Hidden/Universal Render Pipeline/UberPost"
{
    HLSLINCLUDE
		...
		#pragma multi_compile_local_fragment _ _LIGHT_BLUR_KEY
		...
		TEXTURE2D_X(_LightBlur_Texture);
		...
		float4 _LightBlur_Texture_TexelSize;
        float4 _Dithering_Params;
		float _LightBlurIntensity;
        ...
        half4 Frag(Varyings input) : SV_Target
        {
			...
			#if _LIGHT_BLUR_KEY
			{
				half4 lightBlur = SAMPLE_TEXTURE2D_X(_LightBlur_Texture, sampler_LinearClamp, uvDistorted);
				color += lightBlur.xyz * _LightBlurIntensity;
			}
			#endif
