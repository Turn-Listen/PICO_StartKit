...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class PostProcessPass : ScriptableRenderPass
    {
        ...
        public PostProcessPass(RenderPassEvent evt, PostProcessData data, Material blitMaterial)
        {
            ...
            // LightBlur pyramid shader ids - can't use a simple stackalloc in the bloom function as we
            // unfortunately need to allocate strings
            ShaderConstants._LightBlurTemp1 = Shader.PropertyToID("Temp1");
            ShaderConstants._LightBlurTemp2 = Shader.PropertyToID("Temp2");
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            m_LightBlur           = stack.GetComponent<LightBlur>();
        ...
        void Render(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.UberPostProcess)))
            {
                ...
                // LightBlur goes secound
                bool lightBlurActive = m_LightBlur.IsActive();
                if (lightBlurActive)
                {
                    using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.LightBlur)))
                        SetupLightBlur(cmd, GetSource(), m_Materials.uber);
                }
                ...
                // Cleanup //todo
                if (lightBlurActive)
                    cmd.ReleaseTemporaryRT(ShaderConstants._LightBlurTemp2);
        ...
        void SetupLightBlur(CommandBuffer cmd, int source, Material uberMaterial)
        {
            if (m_LightBlur.IsActive())
                uberMaterial.EnableKeyword(ShaderKeywordStrings.LightBlurKey);

            // Start at half-res
            int tw = m_Descriptor.width / m_LightBlur.passDownsample.value;
            int th = m_Descriptor.height / m_LightBlur.passDownsample.value;

            //foreach times
            int mipCount = m_LightBlur.passLoop.value;

            // Material setup
            //float scatter = Mathf.Lerp(0.05f, 0.95f, m_LightBlur.scatter.value);
            var lightBlurMaterial = m_Materials.lightBlur;

            // Prefilter
            var desc = GetCompatibleDescriptor(tw, th, m_DefaultHDRFormat);
            int temp1 = ShaderConstants._LightBlurTemp1;
            int temp2 = ShaderConstants._LightBlurTemp2;
            cmd.GetTemporaryRT(temp1, desc, FilterMode.Bilinear);
            cmd.GetTemporaryRT(temp2, desc, FilterMode.Bilinear);
            Blit(cmd, source, temp1, lightBlurMaterial, 0);

            for (int i = 1; i < mipCount; i++)
            {
                lightBlurMaterial.SetFloat("_Blur", i * m_LightBlur.passBlur.value);

                Blit(cmd, temp1, temp2, lightBlurMaterial, 1);
                Blit(cmd, temp2, temp1, lightBlurMaterial, 1);
            ...
            uberMaterial.SetFloat(ShaderConstants._LightBlurIntensity, m_LightBlur.intensity.value);

            cmd.SetGlobalTexture(ShaderConstants._LightBlur_Texture, ShaderConstants._LightBlurTemp1);
        ...
        class MaterialLibrary
        {
            ...
            public readonly Material lightBlur;
            ...
            public MaterialLibrary(PostProcessData data)
            {
                ...
                lightBlur = Load(data.shaders.lightBlurPS);
        ...
        static class ShaderConstants
        {
            ...
            //LightBlur
            public static readonly int _LightBlur_Texture = Shader.PropertyToID("_LightBlur_Texture");
            public static readonly int _LightBlurIntensity = Shader.PropertyToID("_LightBlurIntensity");
            public static int _LightBlurTemp1;
            public static int _LightBlurTemp2;
        }
